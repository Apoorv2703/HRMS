      ...
      <div className="relative w-full max-w-md">
        <div className="rounded-2xl border border-slate-200 bg-white shadow-2xl p-8 space-y-6">
          ...
          <div className="text-center">
            ...
            <h1 className="text-2xl font-extrabold text-[#101828] tracking-tight">Set New Password</h1>
            <p className="text-sm text-[#475467] mt-1.5">
              {email && <span>Resetting for <strong className="text-[#101828] font-semibold">{email}</strong></span>}
            </p>
          </div>
          ...
          {linkInvalid ? (
            <div className="space-y-4">
              <div className="rounded-xl border border-rose-200 bg-rose-50 p-5 flex gap-3 items-start">
                <AlertTriangle className="h-5 w-5 text-rose-600 mt-0.5 shrink-0" />
                <div>
                  <p className="text-sm font-semibold text-rose-800">Invalid Reset Link</p>
              ...
              <button
                ...
                className="w-full rounded-xl bg-slate-950 hover:bg-slate-900 py-3 text-sm font-bold text-white shadow-lg cursor-pointer"
              >
                Request New Link
              </button>
            </div>
            ...
            <div className="space-y-5">
              <div className="rounded-xl border border-teal-200 bg-teal-50 p-5 flex gap-3 items-start">
                ...
                <div>
                  <p className="text-sm font-semibold text-teal-800">Password Reset Successfully!</p>
                  <p className="text-xs text-teal-700 mt-1 leading-relaxed">{successMsg}</p>
                  {countdown !== null && (
                    <p className="text-xs text-slate-500 mt-2">
                      Redirecting to login in <strong className="text-slate-700">{countdown}s</strong>...
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-300 bg-white pl-10 pr-11 py-3 text-sm text-slate-900 placeholder-slate-400 focus:border-slate-950 focus:outline-none focus:ring-1 focus:ring-slate-950/20 transition duration-200"
                  />
                  <button
                    type="button"
                ...
                {newPassword && (
                  <div className="space-y-1 pt-1">
                    ...
                    <p className={`text-[10px] font-semibold ${
                      strength.score <= 2 ? 'text-rose-600' : strength.score === 3 ? 'text-amber-600' : 'text-teal-600'
                    }`}>
                      {strength.label}
                    </p>
                  </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full rounded-xl border pl-10 pr-11 py-3 text-sm text-slate-900 placeholder-slate-400 bg-white focus:outline-none focus:ring-1 transition duration-200 ${
                      confirmPassword && confirmPassword !== newPassword
                        ? 'border-rose-500/60 focus:border-rose-500 focus:ring-rose-500/20'
                        : 'border-slate-300 focus:border-slate-950 focus:ring-slate-950/20'
                    }`}
                  />
                  <button
                    type="button"
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3.5 space-y-1.5">
                ...
                {[
                  ...
                  <div key={label} className="flex items-center gap-1.5">
                    <div className={`h-1.5 w-1.5 rounded-full ${met ? 'bg-teal-600' : 'bg-slate-300'}`} />
                    <span className={`text-[10px] ${met ? 'text-teal-600' : 'text-slate-500'}`}>{label}</span>
                  </div>
                ))}
              </div>
